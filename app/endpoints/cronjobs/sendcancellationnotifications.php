...
while ($userToNotify = $usersToNotify->fetchArray(SQLITE3_ASSOC)) {
    ...
    if (!$notificationsEnabled) {
        ...
        while ($rowSubscription = $resultSubscriptions->fetchArray(SQLITE3_ASSOC)) {
            ...
            $notify[$rowSubscription['payer_user_id']][$i]['badge'] = 'Cancels today';
        ...
        if (!empty($notify)) {
            ...
            if ($emailNotificationsEnabled) {
                ...
                if ($adminResult !== false) {
                    ...
                    if ($adminRow && !empty($adminRow['server_url'])) {
                        $serverUrl = rtrim($adminRow['server_url'], '/');
                    }
                }
                ...
                foreach ($notify as $userId => $perUser) {
                    ...
                    $brandLogoCid = wallosEmailEmbedBrandLogo($mail);
                    wallosEmailEmbedSubscriptionLogos($mail, $perUser);

                    $mail->Subject = 'Wallos Cancellation Notification';
                    $mail->Body = wallosBuildHtmlEmail(
                        'Cancellation reminder',
                        'These subscriptions are scheduled for cancellation today.',
                        $perUser,
                        $serverUrl,
                        'Open Wallos',
                        $brandLogoCid
                    );
                    $mail->AltBody = wallosBuildPlainTextEmail($message, $perUser);
