...
while ($userToNotify = $usersToNotify->fetchArray(SQLITE3_ASSOC)) {
    ...
    // Check if webhook notifications are enabled and have a completion payload set and get the settings
    ...
    if (!$notificationsEnabled) {
        ...
        // Get current date to check which subscriptions completed on this day
        $currentDate = new DateTime('now');
        $currentDateString = $currentDate->format('Y-m-d');

        $query = "SELECT * FROM subscriptions WHERE user_id = :user_id AND inactive = :inactive AND ended_at = :endedAt AND completion_notified = 0 ORDER BY payer_user_id ASC";
        $stmt = $db->prepare($query);
        $stmt->bindValue(':user_id', $userId, SQLITE3_INTEGER);
        $stmt->bindValue(':inactive', 1, SQLITE3_INTEGER);
        $stmt->bindValue(':endedAt', $currentDateString, SQLITE3_TEXT);
        ...
        if (!empty($notify)) {
            ...
            if ($emailNotificationsEnabled) {
                ...
                foreach ($notify as $userId => $perUser) {
                    $message = "The following subscriptions completed after their last payment:\n";
                    ...
                    $mail->Subject = 'Wallos Subscription Completed';
                    $mail->Body = $message;
            ...
            if ($discordNotificationsEnabled) {
                ...
                if (!$ssrf) {
                    ...
                    foreach ($notify as $userId => $perUser) {
                        ...
                        if ($user['name']) {
                            $message = $user['name'] . ", the following subscriptions completed after their last payment:\n";
                        } else {
                            $message = "The following subscriptions completed after their last payment:\n";
                        }
                    ...
                    foreach ($notify as $userId => $perUser) {
                        ...
                        foreach ($perUser as $subscription) {
                            ...
                            if ($response === false || $httpCode >= 400) {
                                echo "Error sending completion notifications: " . curl_error($ch) . "<br />";
                            } else {
                                echo "Webhook completion notification sent for subscription: " . $subscription['name'] . "<br />";
                            }
            ...
            $markCompletedStmt = $db->prepare("UPDATE subscriptions SET completion_notified = 1 WHERE user_id = :userId AND inactive = 1 AND ended_at = :endedAt AND completion_notified = 0");
            $markCompletedStmt->bindValue(':userId', $userId, SQLITE3_INTEGER);
            $markCompletedStmt->bindValue(':endedAt', $currentDateString, SQLITE3_TEXT);
