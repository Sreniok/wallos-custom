...
async function clearBrowserCacheAndReload() {
  ...
  if (button) {
    button.disabled = true;
    button.textContent = translate("clearing_cache");
  }
  ...
  try {
    ...
    showSuccessMessage(translate("browser_cache_cleared"));
    ...
    console.error(error);
    showErrorMessage(translate("cache_clear_failed"));

    if (button) {
      button.disabled = false;
      button.textContent = translate("clear_browser_cache");
    }
  }
}
