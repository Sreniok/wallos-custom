...
function resetForm() {
  ...
  const lastPaymentDate = document.querySelector("#last_payment_date");
...
function fillEditFormFields(subscription) {
  ...
  const regularPrice = document.querySelector("#regular_price");
  regularPrice.value = subscription.regular_price ?? "";
  ...
  const lastPaymentDate = document.querySelector("#last_payment_date");
  lastPaymentDate.value = subscription.last_payment_date ?? "";
...
function markSubscriptionPaid(event, id) {
  event.stopPropagation();
  event.preventDefault();
