...
function normalizeCronOutput(output) {
  return output.replace(/<br\s*\/?>/gi, '\n').trim();
}

function runNotificationCheck() {
  const resultTextArea = document.getElementById('cronjobResult');
  resultTextArea.value = `${translate('notification_check_running')}\n`;

  runCronJob('updatenextpayment')
    .then(updateOutput => {
      const normalizedUpdateOutput = normalizeCronOutput(updateOutput);
      resultTextArea.value = `[${translate('notification_check_step_update_payments')}]\n${normalizedUpdateOutput}\n\n`;
      return runCronJob('sendnotifications');
    })
    .then(notificationOutput => {
      const normalizedNotificationOutput = normalizeCronOutput(notificationOutput);
      const hasMatches = normalizedNotificationOutput.includes('Subscription:');
      const extraNote = hasMatches ? '' : `\n${translate('notification_check_no_matches')}`;
      resultTextArea.value += `[${translate('notification_check_step_send_notifications')}]\n${normalizedNotificationOutput}${extraNote}\n\n${translate('notification_check_finished')}`;
    })
    .catch(error => {
