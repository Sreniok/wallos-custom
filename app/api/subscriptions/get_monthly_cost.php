...
if ($_SERVER["REQUEST_METHOD"] === "POST" || $_SERVER["REQUEST_METHOD"] === "GET") {
    ...
    // Calculate the monthly cost based on the subscription payment dates in the requested month
    foreach ($subscriptions as $subscription) {
        $startOfMonth = new DateTimeImmutable($year . '-' . str_pad($month, 2, '0', STR_PAD_LEFT) . '-01');
        $endOfMonth = $startOfMonth->modify('last day of this month');

        foreach (getSubscriptionOccurrencesInRange($subscription, $startOfMonth, $endOfMonth) as $paymentDate) {
